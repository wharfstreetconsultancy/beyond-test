...
#sudo docker run -p $NON_SECURE_PORT:8080 -p $SECURE_PORT:8443 -d -e SRV_ENV=$1 -e PGW_ENV=$2 -e SESSION_SECRET=$SESSION_SECRET -e AWS_PROFILE=$3 -e AWS_REGION=$REGION -e SECURE_PORT=$SECURE_PORT -e REST_HOST=$AGW_HOST -e REST_PORT=$AGW_PORT -e AUTH_CLIENT=$USER_POOL_CLIENT -e PGW_HOST=$PGW_HOST -e PGW_CLIENT=$PGW_CLIENT -e PGW_SECRET=$PGW_SECRET --name webapp_marketplace-$newVersion suroor-fashions/webapp-marketplace:$newVersion

...
SRV_ENV=$(aws s3 cp s3://suroor.fashions.config/productmanagement_server.$1.env.properties - --profile $AWS_PROFILE)
...
sudo docker run -p $NON_SECURE_PORT:8081 -p $SECURE_PORT:8444 -d -e SRV_ENV=$1 -e SESSION_SECRET=$SESSION_SECRET -e AWS_PROFILE=$AWS_PROFILE -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e AWS_REGION=$AWS_REGION -e SECURE_PORT=$SECURE_PORT -e REST_HOST=$AGW_HOST -e REST_PORT=$AGW_PORT -e ALLOWED_ORIGIN_PORT=$SECURE_PORT --name webapp_productmanagement-$newVersion suroor-fashions/webapp-productmanagement:$newVersion
#sudo docker run -p $NON_SECURE_PORT:8080 -p $SECURE_PORT:8443 -d -e SRV_ENV=$1 -e PGW_ENV=$2 -e SESSION_SECRET=$SESSION_SECRET -e AWS_PROFILE=$AWS_PROFILE -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e AWS_REGION=$AWS_REGION -e SECURE_PORT=$SECURE_PORT -e REST_HOST=$AGW_HOST -e REST_PORT=$AGW_PORT -e AUTH_CLIENT=$USER_POOL_CLIENT -e PGW_HOST=$PGW_HOST -e PGW_CLIENT=$PGW_CLIENT -e PGW_SECRET=$PGW_SECRET --name webapp_marketplace-$newVersion suroor-fashions/webapp-marketplace:$newVersion
